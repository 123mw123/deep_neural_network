...
def backward_propagation(AL, Y, caches, Lambda):
    ...
    dZ = (1 / m) * (AL - Y)
    A_prev, W, b = cache
    W_reg = np.zeros((W.shape[0],W.shape[1]))
    for i in range(W.shape[0]):
        for j in range(W.shape[1]):
            if (W_reg[i][j] > 0):
                ...
                W_reg[i][j] = -1
    gradients["dW" + str(L)] = np.dot(dZ, A_prev.T) + (Lambda / m) * W_reg
        ...
        for i in range(W.shape[0]):
            for j in range(W.shape[1]):
                if (W_reg[i][j] > 0):
                    ...
                    W_reg[i][j] = -1
        gradients["dW" + str(l + 1)] = np.dot(dZ, A_prev.T) + (Lambda / m) * W_reg
...
def softmax_loss_with_regularization(AL, Y, parameters, Lambda):
    ...
    L = len(parameters) // 2
    for l in range(1, L):
        L1_cost = L1_cost + (np.sum(np.abs(parameters['W' + str(l)])))
    L1_cost = (Lambda / (m)) * L1_cost
    cost = cost + L1_cost
    return cost
...
def model(trainX, trainY, testX, testY, layers, learning_rate, batchSize, iterations,Lambda=0.01,plot_costs=False):
    ...
    if (plot_costs):
        ...
        plt.title('Training and Test Errors for L1 regularization')
        ...
        plt.title('Training and Test Accuracies for L1 regularization')
        plt.xlabel('No of Epochs')
...
model(X, Y, X_test,Y_test,layers,learning_rate=0.03 ,batchSize=100,iterations=1000,Lambda= 0.000001,plot_costs=True)
