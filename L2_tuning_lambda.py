...
Lambda_range = [0.00001,0.00005,0.0001,0.0005,0.001,0.005,0.01,0.05,0.1,0.5,1,5]
train_set_cost = []
train_set_acc = []
validation_set_cost = []
validation_set_acc = []
for Lambda_2 in Lambda_range:
    parameters= model(trainX, trainY,None,None,
                       layers, learning_rate=0.03, batchSize = 100, iterations=100,Lambda = Lambda_2,plot_costs=False)
    AL, _ = forward_propagation(trainX.T,parameters)
    train_cost =  softmax_loss_with_regularization(AL,trainY.T,parameters,Lambda_2)
    train_acc = percentageCorrectPrediction(AL,trainY.T)
    print("cost of train data set for " + str(Lambda_2) + " :" + str(train_cost) + " acc: " + str(train_acc))
    AL, _ = forward_propagation(Val_testX.T, parameters)
    val_cost = softmax_loss_with_regularization(AL, Val_testY.T,parameters,Lambda_2)
    val_acc = percentageCorrectPrediction(AL, Val_testY.T)
    print("cost of cross_validation data set for " + str(Lambda_2) + " :" + str(val_cost) + " acc: " + str(val_acc))
...
plt.title('Tuning lambda for L2 regularization')
plt.xlabel('Lambda')
plt.ylabel('cost')
plt.show()
